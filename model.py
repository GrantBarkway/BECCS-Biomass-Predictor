...
def make_gin_mlp(in_dim, hidden_dim):
    return torch.nn.Sequential(
        torch.nn.LazyLinear(hidden_dim) if in_dim == -1 else torch.nn.Linear(in_dim, hidden_dim),
        torch.nn.ReLU(),
        torch.nn.Linear(hidden_dim, hidden_dim),
    )

class GIN(torch.nn.Module):
    def __init__(self, hidden_channels, out_channels, date_dim=2):
        super(GIN, self).__init__()
        self.conv1 = GINConv(make_gin_mlp(-1, hidden_channels), train_eps=True)
